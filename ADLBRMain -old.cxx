...
int main(int argc, char *argv[])
{
	...
	double noiseScale = sigMuStdseed[1] * 0.025;
	...
	if (diffparam < 0.3)
	{
		if (sourceImageSpacing[0] > 0.58)
		{
		noiseScale = 0.3;
		lambda = 0.0001;
		
		}
		if (sourceImageSpacing[0] <= 0.3)
		{
			noiseScale = 0.5;  //0.2
			lambda = 0.001; //0.2
			//diffusionTime = 1.5;
	...
	else if (0.3 <= diffparam && diffparam < 0.6)
	{
		if (sourceImageSpacing[0] >= 0.58)
		{
			noiseScale = 0.5;
	...
	myfile2 << argv[13] << "," << volumeGt << "," << sigMuStdseed[0] << "," << sigMuStdseed[1] << "," << diffparam << "," << noiseScale << "," << ",";
